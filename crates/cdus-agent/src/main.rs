                            ...
                            Ok(n) => {
                                ...
                                if let Ok(msg) = serde_json::from_slice::<IpcMessage>(&buffer[..n])
                                {
                                    match msg {
                                        ...
                                        IpcMessage::GetSyncedFolders => {
                                            match store_clone.get_synced_folders() {
                                                Ok(folders) => {
                                                    let resp_bytes = serde_json::to_vec(
                                                        &IpcMessage::SyncedFoldersResponse(folders),
                                                    )
                                                    .unwrap();
                                                    let _ = stream.write_all(&resp_bytes);
                                                }
                                        ...
                                        IpcMessage::AddSyncedFolder { path, label } => {
                                            match store_clone.add_synced_folder(&path, label.as_deref()) {
                                                Ok(id) => {
                                                    let resp_bytes = serde_json::to_vec(
                                                        &IpcMessage::Log(format!("Folder added with ID: {}", id)),
                                                    )
                                                    .unwrap();
                                                    let _ = stream.write_all(&resp_bytes);
                                                }
                                        ...
                                        IpcMessage::RemoveSyncedFolder { id } => {
                                            match store_clone.remove_synced_folder(id) {
                                                Ok(_) => {
                                                    let resp_bytes = serde_json::to_vec(
                                                        &IpcMessage::Log("Folder removed".to_string()),
                                                    )
                                                    .unwrap();
                                                    let _ = stream.write_all(&resp_bytes);
                                                }
                                        ...
                                        IpcMessage::GetConflictedFiles { folder_id } => {
                                            match store_clone.get_conflicted_files(folder_id) {
                                                Ok(files) => {
                                                    let resp_bytes = serde_json::to_vec(
                                                        &IpcMessage::ConflictedFilesResponse(files),
                                                    )
                                                    .unwrap();
                                                    let _ = stream.write_all(&resp_bytes);
                                                }
                                        ...
                                        IpcMessage::ResolveConflict { conflict_id, choice } => {
                                            match store_clone.resolve_conflict(conflict_id) {
                                                Ok(_) => {
                                                    let resp_bytes = serde_json::to_vec(
                                                        &IpcMessage::Log(format!("Conflict resolved with choice: {}", choice)),
                                                    )
                                                    .unwrap();
                                                    let _ = stream.write_all(&resp_bytes);
                                                }
